...
namespace SignTinDuc
{
    public class ConnectUsbToken
    {
        ...
        public static Result GetInformationDevices(string arrdata,string messId)
        {
            Result msg = new Result();
            var lstDLL = ScanFolderLoadDll.FindPKCS11DLLs(arrdata);
                                    ...
                                    try
                                    {
                                        ...
                                        if (privateKeyHandles.Count > 0)
                                        {
                                            ...
                                            if (certificate != null && certificate.NotBefore <= DateTime.Now && certificate.NotAfter >= DateTime.Now)
                                            {
                                                var mechanism = factories.MechanismFactory.Create(CKM.CKM_SHA1_RSA_PKCS);
                                                session.Logout();
                                                msg = new Result((int)ResultStatus.OK, JsonConvert.DeserializeObject("{\"Message\":\"Kết nối đến USB Token thành công.\",\"Obj\":\"Kết nối đến USB Token thành công.\"}"), true, true, messId);
                                            }
                                            else
                                            {
                                                session.Logout();
        ...
        public static Result SignXmlUsbToken(string dll, string serial, string dataBase64,string messId)
        {
